...
const MedicineBtn: React.FC<MedicineBtnProps> = ({ medicineName, type, closeBtn,medicineId, backgroundColor = 'default' ,handleClick,onClose}) => {
    ...
    return (
        ...
        >
            ...
            <h4 className='text-sm font-semibold' onClick={(e) => handleClick && handleClick(e, medicineId || null)}>{medicineName}</h4>
            {closeBtn && (
                <button className={`${closeButtonStyles} rounded-full`}>
                    <IoCloseSharp className='p-1'   onClick={() => onClose?.(medicineId!)} />
                </button>
            )}
        </div>
    );
